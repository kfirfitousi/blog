...
In this article, we'll go over the basics of using [next-mdx-remote](https://github.com/hashicorp/next-mdx-remote) to render MDX files with Next.js 13 and TypeScript. The code we'll be writing in this article is available on [GitHub](https://github.com/kfirfitousi/mdx-nextjs-13).

<Callout type="update">

I now recommend using [Contentlayer](https://contentlayer.dev) instead of next-mdx-remote. It takes much less code to set up and it works beautifully with Next.js 13.
...
Note that we're using a custom component called `Card` in the post. We'll define this component later on.

## Defining Types

Let's head over to `/app/page.tsx`, the home page of our app, and where we'll be rendering our post.  
We'll define 2 types, `Frontmatter` and `Post`:

- `Frontmatter` will define which properties we want to extract from the frontmatter of our MDX files. In this case, we'll extract the `title` and `date`.
- `Post` will define the shape of our post. It will contain the serialized MDX content and the frontmatter.
...
Next, we'll define an async function called `getPost` that takes a filepath, and returns a `Post` object by reading the file from the filesystem and serializing the MDX content.
...
Now let's get this post inside our `Home` component. We'll have to change the `Home` component to be an [async server component](https://beta.nextjs.org/docs/data-fetching/fetching#asyncawait-in-server-components) so we can use `await` in the top level of the component.
...
next-mdx-remote provides a component called `MDXRemote`, that is used to render MDX content. To use it, we **must wrap it in a client component**.
...
All this component does is render the `MDXRemote` component.
...
Now let's import the `MdxContent` component into our `Home` component and render it, passing in the serialized content from our post:
...
To make an actual blog, we might want to move our `getPost` function to a file called `/lib/mdx.ts` (or similar) and add two more functions: `getAllPosts`, which returns all of the posts in our blog, and `getAllMdxFiles`, which returns all of the MDX files in the content directory.
...
My blog now uses [Contentlayer](https://contentlayer.io) instead of next-mdx-remote to render MDX files.
