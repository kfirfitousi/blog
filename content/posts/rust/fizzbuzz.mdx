...
We'll start with a naive implementation, and then we'll refactor it to make it more reusable. Along the way, we'll learn about Rust's enums and traits and use TypeScript for comparison.
...
This is pretty straightforward. We loop through the numbers from `1{:rust}` to the given `number{:rust}`, and print the appropriate string. However, it's not very reusable. What if we wanted to print `"Foo"{:rust}` when the number is divisible by 7 but not 3 or 5? Our code would get messy pretty quickly.
...
The `match{:rust}` expression used in the `new{:.fn}` method is functionally equivalent to the `if{:rust}` statement we used in our naive implementation, it's just more readable in my opinion.
...
Notice how our match expression is derived directly from the requirements table.
