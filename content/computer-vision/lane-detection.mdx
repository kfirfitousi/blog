...
date: "2022-12-10"
tags: ["computer vision", "opencv", "python", "hough transform"]
---
...
The Hough Transform uses the **polar coordinate system** to represent lines. In this system, a line is represented by the equation **ρ = xcosθ + ysinθ**, where **ρ** is the distance from the origin and **θ** is the angle between the line and the x-axis, measured in radians.

![Polar coordinate system {{ w: 700, h: 393, cap: "The polar coordinate system. Note that in images, the origin (0,0) sits at the top left, and not the bottom left as you might expect." }}](/images/computer-vision/lane-detection/polar-coordinates.webp)

Left lane marking are either vertical or slanted to the right, so in this representation, their **θ** will be between 0 and 7π/18 radians (70 degrees). Right lane markings are either vertical or slanted to the left, so their **θ** will be between 11π/18 radians (110 degrees) and π radians (180 degrees).  
...
- `ρ = 1` - This is the resolution of the **ρ** parameter. The higher the value, the more accurate the lines will be, but the more lines will be detected.
- `θ = np.pi/180` - This is the resolution of the **θ** parameter. The higher the value, the more accurate the lines will be, but the more lines will be detected.
- `threshold = 100` - This is the minimum number of pixels that need to be on a line for it to be detected.
- `min_theta` and `max_theta` - These are the minimum and maximum values of **θ** that we want to detect.
...
We don't want to draw the lines directly on the frame, because we want to control their opacity. We can do this by creating a new frame to draw on, and then use the `addWeighted` function to add the new frame to the original frame with a specified opacity.

```python
lanes_image = np.zeros_like(frame)
```

This creates a new frame that is the same size as the original frame, but with all pixels set to black.
...
To draw the lines, we use the `line` function, which takes the frame to draw on, the start and end points, the color, and the line width as arguments.
...
Finally, we use the `addWeighted` function to add the new frame to the original frame with a specified opacity.
...
When a lane change is detected, we discard the previous left and right lines and the previous **x** values, since they are no longer relevant. Also, we reset the `lane_change_frames_shown` variable, which keeps track of how many frames have passed since the message was displayed.
